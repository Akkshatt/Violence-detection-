...
transfer_values_size = transfer_layer.output.shape[1]


print("The input of the VGG16 net have dimensions:", image_model.input.shape[1:3])
...
def make_files(n_files):
    ...
    try:
        ...
        print("Iterator reached its end. Processing complete.")
...
def label_video_names(in_dir):
    ...
    for current_dir, dir_names, file_names in os.walk(in_dir):
        for file_name in file_names:
            if file_name.startswith('fi'):
                labels.append([1, 0])
                names.append(file_name)
            elif file_name.startswith('no'):
                labels.append([0, 1])
                names.append(file_name)
    ...
    def label_video_names(in_dir):
    # Your code to generate 'c'
    
      if not c:
        print("No data available to label")
        return [], []
      else:
        names, labels = zip(*c)
        return names, labels
...
model = Sequential()
model.add(Input(shape=(n_chunks, chunk_size)))  # Use Input(shape) as the first layer
